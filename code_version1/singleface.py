...
EMOTIONS = ['anger', 'contempt', 'disgust', 'fear', 'happy', 'sadness', 'surprise', 'neutral']


# initialize the cascade
cascade_classifier = cv2.CascadeClassifier('../classifier/haarcascade_frontalface_default.xml')  
...
network = EmModel('../face_data', 'model')
network.load_model()
...
while True:
    ...
    ret, frame = cap.read()
    facecasc = cv2.CascadeClassifier('../classifier/haarcascade_frontalface_default.xml')
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    faces = facecasc.detectMultiScale(gray, 1.3, 5)

    if len(faces) > 0:
        face = format_image(frame)
        result = network.forward(torch.tensor(face))
        # Display the emoji
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(frame,EMOTIONS[result],(10,360), font, 2,(255,255,255),2,cv2.LINE_AA) 
        face_image = feelings_faces[result]
        # print(face_image[:,:,3])
    ...
    if not len(faces) > 0:
        ...
        a = 1
        cv2.imshow('Video', cv2.resize(frame,(960, 640),fx=2,fy=2,interpolation = cv2.INTER_CUBIC))  
    else:
